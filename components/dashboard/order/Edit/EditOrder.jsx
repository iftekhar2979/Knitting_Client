...
const EditOrder = ({id,data:orderInfo}) => { 
    ...
    const [editOrder,{isLoading:editOrderLoading,isError:editingOrderError}]=useEditOrderMutation(id)
const { data: product, isLoading: productLoading, error: productError, isError: productIsError } = useGetProductQuery()
    ...
    const handleInputDropdown = (e) => {
        ...
        const findCompanyLocation = companyInformation?.find((item, index) => {
        ...
        <div className="w-full my-4 flex flex-col  justify-center">
            <div className="grid grid-cols-2 gap-6 mx-6 ">
                ...
                <div className="my-2">
                    <div className="grid w-full max-w-sm items-center gap-1.5">
                        ...
                        <Input type="number" id="Quantity" placeholder="Quantity" defaultValue={orderInfo?.orderQuantity} onWheel={() => document.activeElement.blur()} onChange={(e) => setQuantity(e.target.value)} />
                    </div>
                </div>
            </div>
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="w-full  rounded-xl  ">
                    ...
                    <Button type="submit" className="mx-auto p-auto">{editOrderLoading?"Editing" : "Edit"}</Button>
                    </div>
                </form>
            </Form>
            {editingOrderError && <Error error={"Order did not Edited Successfully!!, Please Try again Later !!!"}/>}
            <hr/>
          <h2  className="text-center text-2xl underline font-serif">Order Details Edit</h2>
